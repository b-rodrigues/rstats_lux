...
title: "Scraping data from the local election 2017 results"
...
At the beginning, it was a private code but I thought that it could be another great scraping example after the excellent post written by my colleague Bruno Rodriguez about scraping data from STATEC public tables.

So let’s get started with the loading packages.
...
After clicking on different communes, we notice that the URL have the same format. They have 3 parts :<br>
...
There are 103 communes, so we have to put all of them in a list of vector. We scrape the 103 communes in one vector via the script bellow :
...
As you can see, it’s looking good but we are not completely satisfied because we would like to transpose data to have one result in one column. To this end, we use the library(tidyr).
...
I hope you enjoyed reading this post. Don't hesitate to contact us for any questions.
Follow us on [twitter](https://twitter.com/rdata_lu?lang=en) and subscribe to our [youtube channel.](https://www.youtube.com/channel/UCbazvBnJd7CJ4WnTL6BI6qw) See you for the next post ;) <br>
